...
const loadSessionModule = async (sessionCookieSecure: 'true' | 'false' = 'true') => {
  vi.resetModules();
...
describe('session cookie security detection', () => {
  it('keeps Secure cookies on HTTPS requests', async () => {
    const { shouldUseSecureSessionCookie } = await loadSessionModule('true');
  ...
  it('honors explicit insecure-cookie configuration for local HTTP deployments', async () => {
    const { shouldUseSecureSessionCookie } = await loadSessionModule('false');
